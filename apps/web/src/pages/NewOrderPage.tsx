...
export function NewOrderPage() {
  ...
  const calculateProduct = (
    ...
    if (orderBy === 'trays') {
      ...
      // Halalnivore special rule: if 1 extra tub, pack as 4
      if (rules.extraTubRule === 'pack4') {
        const remainder = tubs % tubsPerBox;
        if (remainder === 1 && tubs > tubsPerBox) {
          // We have 1 extra, so make the last box have 4 instead
          boxes = Math.floor(tubs / tubsPerBox);
          if (remainder > 0) {
            // Actually recalculate: pack most as 3, last as 4 if possible
            const fullBoxes = Math.floor((tubs - 1) / tubsPerBox);
            const remaining = tubs - (fullBoxes * tubsPerBox);
            boxes = fullBoxes + (remaining > 0 ? 1 : 0);
          }
        } else {
          boxes = Math.ceil(tubs / tubsPerBox);
        }
